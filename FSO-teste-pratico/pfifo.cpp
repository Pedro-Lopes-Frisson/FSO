...
void insert_pfifo(PriorityFIFO *pfifo, int id, int priority) {
  ...
  require(pfifo->is_closed || !full_pfifo(pfifo),
          "open FIFO is full"); // IMPORTANT: in a shared fifo, it may not
...
int retrieve_pfifo(PriorityFIFO *pfifo) {
  ...
  require(pfifo->is_closed || !empty_pfifo(pfifo),
          "open FIFO is empty"); // IMPORTANT: in a shared fifo, it may not
                                 // result from a program error!
  ...
  ensure((result >= 0 && result <= MAX_ID) || is_closed_pfifo(pfifo),
         "OPEN FIFO with an invalid id"); // a false value indicates a program
                                          // error
