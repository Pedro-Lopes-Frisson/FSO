...
ThreadArgs ta_patient [MAX_PATIENTS];
ThreadArgs ta_doctors[MAX_DOCTORS];
ThreadArgs ta_nurses[MAX_NURSES];
...
int doctor_iteration(int id) // return value can be used to request termination
{
  ...
  check_valid_patient(patient);
  printf("\e[32;01mDoctor %d: treat patient %d - %s\e[0m\n", id, patient,
         hd->all_patients[patient].name);
  random_wait();
...
void patient_wait_end_of_consultation(int id) {
  ...
  printf("\e[0;35mPatient %s (number %d): health problems treated\e[0m\n",
         hd->all_patients[id].name, id);
...
void patient_life(int id) {
  ...
  // nurse_iteration(0);  // TODO point: to be commented/deleted in concurrent
  // version doctor_iteration(0); // TODO point: to be commented/deleted in
  // concurrent version
...
void *patient_thread(void *arg) {
  ThreadArgs *id =
      (ThreadArgs *)arg; // we know arg is a pointer to a ProducerData structure
  patient_life(id->id);
  thread_exit(id);
  return NULL;
}
...
/* ************************************************* */
pthread_t thr_patients[MAX_PATIENTS], thr_doctors[MAX_DOCTORS],
    thr_nurses[MAX_NURSES];
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < npatients; i++) {
    ta_patient[i].id = i;
    thread_create(&thr_patients[i], NULL, patient_thread,
                  (void *)&ta_patient[i]);
  }
