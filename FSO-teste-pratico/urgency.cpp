...
void* patient_thread(void * arg){
    int *id = (int*)arg;   // we know arg is a pointer to a ProducerData structure
    patient_life(*id);
    thread_exit(NULL);
    return NULL ;
}
...
int main(int argc, char *argv[])
{
   ...
   for(int i = 0; i < npatients; i++)
   {
      thread_create(&thr_patients[i], NULL,patient_thread, (void *)&i);
   }
   for(int i = 0; i < ndoctors; i++)
   {
      thread_create(&thr_doctors[i], NULL,doctor_thread, (void *)&i);
   }
   for(int i = 0; i < nnurses; i++)
   {
      thread_create(&thr_doctors[i], NULL,nurse_thread, (void *)&i);
   }
